...
int main(int argc, char* argv[])
{
	...
	printf("H^{2048}: ");
	init(&ctx, HASH512);
	u8 messages1[4][sizeof(message1) / 4] = { 0 };
	for (int i = 0; i < 4;i++) {
		for (int j = 0;j < sizeof(message1) / 4; j++) {
			messages1[i][j] = message1[(i + 1) * j];
		}
		stribog(&ctx, messages1[i], sizeof(message1) / 4);
		print_hash(&ctx);
	}
	putchar('\n');
	...
	printf("H^{2048}: ");
	init(&ctx, HASH512);
	u8 messages2[4][sizeof(message2) / 4] = { 0 };
	for (int i = 0; i < 4;i++) {
		for (int j = 0;j < sizeof(message2) / 4; j++) {
			messages2[i][j] = message2[(i + 1) * j];
		}
		stribog(&ctx, messages2[i], sizeof(message2) / 4);
		print_hash(&ctx);
	}
	putchar('\n');
